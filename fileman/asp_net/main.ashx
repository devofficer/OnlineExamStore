...
public class RoxyFilemanHandler : IHttpHandler, System.Web.SessionState.IRequiresSessionState
{
    ...
    protected string GetFileType(string ext){
        ...
        if(ext == ".jpg" || ext == ".jpeg" || ext == ".png" || ext == ".gif")
            ret = "image";
        else if(ext == ".swf" || ext == ".flv" || ext == ".mp4" || ext == ".avi" || ext == ".WebM" || ext == ".mp3")
            ret = "flash";
        else if(ext == ".pdf" || ext == ".doc" || ext == ".docx" || ext == ".xls" || ext == ".xlsx" || ext == ".txt")
            ret = "document";
        else
            ret = "all";
        return ret;
    ...
    {
        if (FixPath(path).IndexOf(GetFilesRoot()) != 0 || path.IndexOf("../") > -1 || path.IndexOf("..\\") > -1)
        {
    ...
    protected void Upload(string path)
    {
        ...
        try{
            for(int i = 0; i < HttpContext.Current.Request.Files.Count; i++){
                ...
                {
                    FileInfo f = new FileInfo(HttpContext.Current.Request.Files[i].FileName);
